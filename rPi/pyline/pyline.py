...
class Line:
    ...
    def from_string(self, text_line):
        ...
        params = [float(n) for n in text_line.split(" ")]
        return Line(params)

    def __init__(self, params):
        """
        :param text_line: A list containing of numbers separated by
            spaces. Format: x1, y1, x2, y2, width, p, -log10(NFA)
        :type text_line: list
        """
        self.x1, self.y1, self.x2, self.y2, self.width, self.p, \
            self.NFA = params
...
def toGray(surf):
    ...
    img = img.convert("L")
    img = img.convert("RGB")
    newsurf = pygame.image.fromstring(img.tostring(), surf.get_size(),
                                      "RGB", False)
    return newsurf
...
def testVideo():
    ...
    while playing:
        ...
        if rf != f or pygame.key.get_pressed()[pygame.K_ESCAPE]:
            ...
            if (f % 1) == 0:  # skip some frames for faster processing
                ...
                lines = [Line(seg) for seg in segs]
                angles = [l.angle() for l in lines]
                ...
                pygame.draw.line(grayvideosurf, (255, 0, 0), (x0, y0),
                                 (x1, y1), 5)
    ...
    # generate movie
    #os.system("""mencoder "mf://frame*.tga" -mf fps=25 -ovc lavc
    #-lavcopts vhq:vbitrate=600 -o movie.avi""")
    # delete the frames
